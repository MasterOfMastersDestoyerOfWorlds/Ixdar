#!/usr/bin/env bash
# Sweep CrossField paper-faithful parameters and record (count, agree) per run.
# Usage: bash tmp/sweep.sh > tmp/sweep.csv
set -uo pipefail

cd "$(dirname "$0")/.."

echo "h,tauMin,kScale,count,agree,refOnly,genOnly,energy,wallSec"

for h in 0.025 0.03 0.04 0.05 0.06 0.08; do
  for tau in 0.6 0.7 0.8 0.9; do
    for k in 0.05 0.1 0.2; do
      out=$(mvn surefire:test \
        -Dsurefire.failIfNoSpecifiedTests=false \
        -Dtest='CrossFieldBuildProfileTest' \
        -DcrossField.hFraction="$h" \
        -DcrossField.tauMin="$tau" \
        -DcrossField.kScale="$k" \
        -q 2>&1)
      ...
      echo "$h,$tau,$k,${count:-NA},${agree:-NA},${refOnly:-NA},${genOnly:-NA},${energy:-NA},${wall:-NA}"
    done
  done
done
