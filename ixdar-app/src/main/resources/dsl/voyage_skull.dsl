...
eye_x_center = boolean_math(a=eye_x_left.value, b=eye_x_right.value, operation=OR)

eye_y_min = compare(a=pos_xyz.y, b=0.0, mode=GREATER)
eye_y_max = compare(a=pos_xyz.y, b=0.5, mode=LESS)
eye_y_range = boolean_math(a=eye_y_min.value, b=eye_y_max.value, operation=AND)

eye_sel = boolean_math(a=eye_x_center.value, b=eye_y_range.value, operation=AND)
...
nasal_x_center = boolean_math(a=nasal_x_min.value, b=nasal_x_max.value, operation=AND)
nasal_y_min = compare(a=pos_xyz.y, b=0.0, mode=GREATER)
nasal_y_max = compare(a=pos_xyz.y, b=0.5, mode=LESS)
nasal_y_range = boolean_math(a=nasal_y_min.value, b=nasal_y_max.value, operation=AND)

nasal_sel = boolean_math(a=nasal_x_center.value, b=nasal_y_range.value, operation=AND)
...
brow_y_range = boolean_math(a=brow_y_min.value, b=brow_y_max.value, operation=AND)
brow_x_min = compare(a=pos_xyz.x, b=0.6, mode=GREATER)
brow_x_max = compare(a=pos_xyz.x, b=0.6, mode=LESS)
brow_x_range = boolean_math(a=brow_x_min.value, b=brow_x_max.value, operation=AND)

brow_sel = boolean_math(a=brow_y_range.value, b=brow_x_range.value, operation=AND)
...
jaw_x_range = boolean_math(a=jaw_x_min.value, b=jaw_x_max.value, operation=AND)

jaw_sel = boolean_math(a=jaw_y_max.value, b=jaw_x_range.value, operation=AND)
