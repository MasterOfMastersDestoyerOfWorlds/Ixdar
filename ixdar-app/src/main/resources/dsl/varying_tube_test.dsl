...
path_r = resample_curve(curve=path.curve, length=0.08)

# Radius varies: thin(0.2) → bulge(0.8) → thin(0.2)
radius_fc = float_curve(points="0,0.2, 0.5,0.8, 1.0,0.2")

# Sweep with varying radius
tube = curve_to_mesh(curve=path_r.curve, radius=1.0, radius_closure=radius_fc.closure, resolution=16, fill_caps=true)
