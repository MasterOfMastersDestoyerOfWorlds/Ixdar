...
abs_ndz = float_math(operation=ABSOLUTE, a=n_dot_z.result, b=0.0)
is_z_aligned = compare(a=abs_ndz.result, b=0.99, mode=GREATER_THAN)
reference = switch_vector(switch=is_z_aligned.value, false=z_axis.vector, true=x_axis.vector)
t1_raw = vector_math(operation=CROSS_PRODUCT, a=norm.vector, b=reference.vector)
...
u_cos = float_math(operation=MULTIPLY, a=local_u.result, b=cos_a.result)
v_sin = float_math(operation=MULTIPLY, a=local_v.result, b=sin_a.result)
rotated_u = float_math(operation=SUBTRACT, a=u_cos.result, b=v_sin.result)
u_sin = float_math(operation=MULTIPLY, a=local_u.result, b=sin_a.result)
v_cos = float_math(operation=MULTIPLY, a=local_v.result, b=cos_a.result)
...
pos_along = switch_float(switch=nearest_vert.value, false=cell_x.result, true=cell_y.result)
...
paths = input_shortest_edge_paths(end=is_corner.value, edge_cost=edge_cost.result)
edge_sel = edge_paths_to_selection(start=start_verts.value, next_vertex=paths.next_vertex)
...
resampled = resample_curve(curve=curve_geo.geometry, length=stitch_spacing.result)

# --- Stitch beads ---
bead = icosphere(radius=1.0, subdivisions=2)
tangent = input_tangent()
align_rot = align_rotation_to_vector(vector=tangent.vector)
stitch_scale = combine_xyz(x=stitch_size.result, y=stitch_size.result, z=stitch_size.result)
instanced = instance_on_points(points=resampled.geometry, instance=bead.mesh, rotation=align_rot.rotation)
...
quilt_out = switch_geometry(switch=stitches.value, false=quilted.geometry, true=joined.geometry)
