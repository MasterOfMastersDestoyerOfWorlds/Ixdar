 ...
 * copy, sizing the rectangle from the sides' quantized lengths floored to
 * {@link #MIN_RECTANGLE_SIDE}.
 *
 * <p>
...
public final class PatchRegionMapper {
    ...
    /**
     * Smallest rectangle side length, so a unit-quantized side still maps to a real
     * rectangle.
     */
    ...
    /**
     * The copy vertices along one side of a patch, from the side's start corner up
     * to but not including the next corner, oriented so consecutive sides join
     * without repeating a vertex.
     *
     * @param patchId patch to read
     * @param side    side index in {@code [0, 4)}
     * @return the side's copy vertex ids in walking order, excluding the trailing
     *         corner
     */
