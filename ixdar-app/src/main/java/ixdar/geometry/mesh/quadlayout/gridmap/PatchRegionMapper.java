...
public final class PatchRegionMapper {
     ...
     * @throws IllegalStateException when the patch has no region, a boundary vertex
     *                               is missing from the region, or a face is not a
     *                               triangle
    ...
    public PatchRectangleMap mapPatch(int patchId) {
        ...
        for (int side = 0; side < EmbeddedPatch.SIDES; side++) {
            ...
            sideBreakLoopIndex[side][lastBreak] = sideBreakLoopIndex[(side + 1) % EmbeddedPatch.SIDES][0];
        }

        requireSimpleBoundary(patchId, boundaryLoop, denseCopyVertex);
        double width = requireOppositeSidesEqual(patchId, 0);
        double height = requireOppositeSidesEqual(patchId, 1);
    ...
    /**
     * Checks the patch's boundary loop visits every vertex once: a repeat pins one
     * vertex to two rectangle positions, so the region is a pinched disk and no
     * bijective map onto the rectangle exists — an upstream contraction defect.
     *
     * @param patchId         patch being mapped, for the message
     * @param boundaryLoop    dense vertex indices around the region boundary
     * @param denseCopyVertex copy vertex id of each dense index, for the message
     * @throws IllegalStateException when any boundary vertex repeats
     */
    private void requireSimpleBoundary(int patchId, List<Integer> boundaryLoop,
            List<Integer> denseCopyVertex) {
        Map<Integer, Integer> firstVisit = new HashMap<>();
        for (int index = 0; index < boundaryLoop.size(); index++) {
            Integer earlier = firstVisit.putIfAbsent(boundaryLoop.get(index), index);
            if (earlier != null) {
                throw new IllegalStateException("patch " + patchId + " boundary is pinched: copy"
                        + " vertex " + denseCopyVertex.get(boundaryLoop.get(index))
                        + " appears at loop positions " + earlier + " and " + index + " of "
                        + boundaryLoop.size() + "; the region is not a disk, so no bijective"
                        + " rectangle map exists and the contraction upstream must stop"
                        + " producing it");
            }
        }
    }
    ...
    /**
     * The dense index of a boundary vertex, which must already have been seen in
     * the region's faces.
     *
     * @param patchId           patch being mapped, for the message
     * @param side              side being walked, for the message
     * @param copyVertex        copy vertex on the patch's boundary
     * @param denseByCopyVertex map from copy vertex id to dense index, filled from
     *                          the region
     ...
     * @param denseByCopyVertex map from copy vertex id to dense index, extended
     *                          here
