...
public class FixedCutTwoMatches extends FixedCut {
    ...
    public CutMatchList findCutMatchListFixedCut() throws SegmentBalanceException {
        ...
        if (!bothKnotPointsInside && !neighborSegments.contains(knot.getSegment(cp1, external2)) && !wouldOrphan) {
            ...
            // Triple<ArrayList<Segment>, Segment, Pair<VirtualPoint, VirtualPoint>> value =
            // findSimpleMatch(cutSegment1);
            result = new CutMatchList(shell, sbe, c.superKnot);
            shell.buff.add("ign " + c.lowerMatchSegment + " " + knot.getSegment(cp1, external2));

            // c.innerNeighborSegmentLookup.values().toArray(new
            // Segment[innerNeighborSegmentLookup.size()])
            result.addCutMatch(new Segment[] {},
                    new Segment[] { knot.getSegment(c.lowerCutSegment.getOther(c.lowerKnotPoint), external2), }, c, "FixedCutTwoMatchesKnotPointOutsideSimpleCut");
        ...
        if (bothKnotPointsInside && !neighborSegments.contains(knot.getSegment(cp1, external2)) && !wouldOrphan) {
            ...
            overlapping = 2;
            Triple<Set<Segment>, Set<Segment>, Pair<VirtualPoint, VirtualPoint>> value = findSimpleMatch(cutSegment1);
            ...
            result.addCutMatch(new Segment[] {}, new Segment[] { m }, flatCut, "FixedCutTwoMatchesKnotPointsInsideSimpleCut");
            minDelta = result.delta;
        }
                ...
                if (bothKnotPointsInside) {
                    ...
                    if (numUnique == 1) {
                        ...
                        cutMatch2.addCutMatch(new Segment[] { cutSegment2 },
                                ...
                                c, "FixedCutTwoMatchesKnotPointsInsideOneUnique");
                    ...
                    if (numUnique == 2) {
                        ...
                        if (canMatch3) {
                            ...
                            cutMatch3.addCutMatch(new Segment[] { cutSegment2 },
                                    ...
                                    c, "FixedCutTwoMatchesKnotPointsInsideTwoUnique");
