...
public class Shell extends LinkedList<Point2D> {
	 ...
	 * @param c the color to draw the shell
	 */
	...
	/* All code is currently unused but being kept for history and potential future use
		...
		public Shell getParent() {
			...
			result.child = result.child.child; //seems odd should it not be this.child.child? should we be updating order here?
			return result;
		}
		...
		public Shell collapseShellOntoParent() {
			...
			result.child = result.child.child; //seems odd should it not be this.child? should we be updating order here?
			return result;
		}
