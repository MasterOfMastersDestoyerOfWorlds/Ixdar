...
(Something to think about: a graph with only Runs and no Knots may break the triangle inequality? p-sure that 1->4, 4->5, 5->1 breaks triangle inequality because it implies that 4->1 is longer than 4->5 and 4->5 is longer than 5->1 and 5->1 is longer than 1->4 meaning there is no longest segment)
